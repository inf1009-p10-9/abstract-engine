...
abstract class CurrencyToItemOffer<IT extends Object,
    ...
    public <T extends IOfferRequest<?>> boolean isTransactionViable(IWalletBagImmutableOwnership wallets,
        ...
        /*if (!sourceWallet.isDebitViable())
            return false;
        System.out.println("[isTransactionViable] Source wallet reports debit is viable given total price.");
        if (!targetWallet.isItemAcceptable(targetItem)) {
            return false;
        }
        System.out.println("[isTransactionViable] Target wallet reports item is acceptable.");*/
        return true;
    }
